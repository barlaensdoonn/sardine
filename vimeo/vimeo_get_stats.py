...
def call_api(caller, call):
    ...
    while next_page:
        ...
        for i in range(len(data)):
            ...
            tupled = (data[i]['name'], play, like)
            vid_list.append(tupled)

        next_page = videos.json()['paging']['next']

    return (vid_list, plays, likes)


def write_results(vid_list, plays, likes):
    with open(vimeo_credents.stats_output_file, 'w') as out:
        csv_out = csv.writer(out)
        csv_out.writerow(['VIDEO', 'PLAYS', 'LIKES'])

        for row in vid_list:
            csv_out.writerow(row)

        csv_out.writerow([])
        csv_out.writerow(['TOTAL PLAYS', 'TOTAL LIKES'])
        csv_out.writerow([plays, likes])
